...
const Page = () => {
  ...
  const [selectedSpeech, setSelectedSpeech] = React.useState<Speech | null>(
    speeches.filter((speech: Speech) => currentUser?.id === (speech.speechID).substring(0, 4))[0] || null
  );
  ...
  const [filteredSpeeches, setFilteredSpeeches] = React.useState<Speech[]>(speeches);
  ...
  useEffect(() => {
    ...
    setFilteredSpeeches(
        speeches.filter((speech: Speech) => ( speech.speechID.substring(0, 4) === currentUser?.id) && (!search ? true: (speech.title.toLowerCase().includes(search.toLowerCase())))))}
            ...
            <section className='w-full pr-8 items-center justify-center'>
                ...
                <div className='flex justify-center items-center'>
                  ...
                  <nav className='inline-block h-8 ml-4'>
                    {selectedSpeech?.tags?.map((tag, index) => (
                      <div key={index} className='cursor-pointer transition-transform text-center mx-2 inline-block'>
                        <p className='w-12 text-2xl h-8 bg-white rounded-lg'>{tag}</p>
                      </div>
                    ))}
                  </nav>
                </div>
              ...
              <div className='flex justify-center items-center mt-4'>
                <button
                  ...
                  onClick={() => {
                    selectedSpeech ? toast("Speech Updated") : toast("Speech Created");
                    handleAddSpeech();
                  }}>
